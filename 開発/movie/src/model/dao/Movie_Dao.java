...
public class Movie_Dao extends DaoBase{

	//映画一覧を表示//
	public List<Movie_List_Beans>getMovieView(){
		if(con==null) {
			return null;
		}
		...
		try {
			...
			while(rs.next()) {
				Movie_List_Beans beans = new Movie_List_Beans();
	...
	public List<Movie_List_Beans> searchMovie(String searchmovie) {
		...
		try{
			...
			while(rs.next()) {
				Movie_List_Beans searchbeans = new Movie_List_Beans();
	...
	//映画詳細を表示するメソッド//
	public List<Movie_Detail_Beans> getMovieDetail(int movie_id) throws SQLException{
		if(con == null) {
			//接続していない場合はエラー//
			return null;
		}
		List<Movie_Detail_Beans> detailList = new ArrayList<Movie_Detail_Beans>();
		Movie_Detail_Beans movieDetailBeans = new Movie_Detail_Beans();
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			//select文実行//
			...
			while(rs.next()) {
				...
				detailList.add(movieDetailBeans);
			}
		} catch(SQLException e) {
			e.printStackTrace();
		} finally {
